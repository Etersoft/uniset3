    ...
    UniSetObject(id)
{
    ...
    runlock = unisetstd::make_unique<uniset3::RunLock>(lockfile.str());
...
//--------------------------------------------------------------------------------------------
void UWebSocketGate::UWebSocket::fill_json( Poco::JSON::Object::Ptr& json, const uniset3::umessage::SensorMessage* sm, const std::shared_ptr<sinfo>& si )
{
    ...
    if( !calibr )
        calibr = uniset3::json::make_child(json, "calibration");
...
//--------------------------------------------------------------------------------------------
Poco::JSON::Object::Ptr UWebSocketGate::UWebSocket::to_json( const umessage::SensorMessage* sm, const std::shared_ptr<sinfo>& si )
{
    Poco::JSON::Object::Ptr json = new Poco::JSON::Object();
...
{
    myinfoV(3) << myname << "(deactivateObject):.." << endl;
    terminate();
...
// -----------------------------------------------------------------------------
void UWebSocketGate::sysCommand( const uniset3::umessage::SystemMessage* _sm )
{
    ...
    resp(_resp)
{
    ...
    wbufpool = make_unique<Poco::ObjectPool< uniset3::UTCPCore::Buffer >>(jpoolCapacity, jpoolPeakCapacity);
}
// -----------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
void UWebSocketGate::UWebSocket::freeze( uniset3::ObjectId id, long value )
{
...
// -----------------------------------------------------------------------------
void UWebSocketGate::UWebSocket::unfreeze( uniset3::ObjectId id )
{
...
void UWebSocketGate::UWebSocket::onCommand( std::string_view cmdtxt )
{
    ...
    if( cmd == "set" )
    {
        ...
        auto idlist = uniset3::getSInfoList_sv(params, uniset_conf());
