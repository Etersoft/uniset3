...
//--------------------------------------------------------------------------------------------
void uniset3::ClickHouseTagsConfig::load( const std::shared_ptr<uniset3::Configuration> conf
        , const std::string& ff
        , const std::string& fv)
{
    ...
    {
        if( !uniset3::check_filter(it, ff, fv) )
            continue;
...
//--------------------------------------------------------------------------------------------
void ClickHouseTagsConfig::loadTagsMap( uniset3::UniXML::iterator it )
{
    tagsmap = initTagsMap(it);
}
//--------------------------------------------------------------------------------------------
void ClickHouseTagsConfig::initFromItem( const std::shared_ptr<uniset3::Configuration> conf
        , const uniset3::UniXML::iterator& it )
{
...
//--------------------------------------------------------------------------------------------
void ClickHouseTagsConfig::initFromTags( const std::shared_ptr<uniset3::Configuration>& conf
        , UniXML::iterator it
        , const std::string& s_tags )
{
    uniset3::ObjectId myId = it.getIntProp("id");
    std::string s_id = it.getProp("clickhouse_dyn_sid");
    uniset3::ObjectId sid = conf->getSensorID(s_id);
...
//--------------------------------------------------------------------------------------------
void ClickHouseTagsConfig::initFromValues( const std::shared_ptr<uniset3::Configuration>& conf
        , UniXML::iterator it
...
//--------------------------------------------------------------------------------------------
void ClickHouseTagsConfig::initFromTagsList( const std::shared_ptr<uniset3::Configuration>& conf
        , uniset3::UniXML::iterator it
        , uniset3::UniXML::iterator itList )
{
    uniset3::ObjectId myId = it.getIntProp("id");

    for( ; itList.getCurrent(); ++itList )
    {
        uniset3::ObjectId sid = conf->getSensorID(itList.getProp("name"));
...
//--------------------------------------------------------------------------------------------
ClickHouseTagsConfig::StateMap::iterator ClickHouseTagsConfig::addState( uniset3::ObjectId sid, long defval )
{
...
std::vector<ClickHouseTagsConfig::Tag> ClickHouseTagsConfig::parseTags( const std::string& s_tags, const char sep)
{
    ...
    if( sep == 0 )
    {
        auto tval = uniset3::split(s_tags, '=');
    ...
    {
        auto tlist = uniset3::split(s_tags, sep);

        for( const auto& t : tlist )
        {
            auto tval = uniset3::split(t, '=');
...
ClickHouseTagsConfig::RangeList ClickHouseTagsConfig::initRangeList( UniXML::iterator it )
{
    ...
    for( ; it.getCurrent(); ++it )
    {
        ...
        if( l[0] == "min" )
            ...
            r.vmin = uniset3::uni_atoi(l[0]);

        if( l[1] == "max" )
            r.vmax = std::numeric_limits<long>::max();
        else
            r.vmax = uniset3::uni_atoi(l[1]);
...
//--------------------------------------------------------------------------------------------
ClickHouseTagsConfig::TagList::iterator ClickHouseTagsConfig::addTagsInfo( TagList& lst, uniset3::ObjectId sid )
{
...
//--------------------------------------------------------------------------------------------
bool ClickHouseTagsConfig::updateTags( uniset3::ObjectId id, long value )
{
...
//--------------------------------------------------------------------------------------------
std::vector<uniset3::ObjectId> ClickHouseTagsConfig::getAskedSensors() const
{
    std::vector<uniset3::ObjectId> lst;
...
//--------------------------------------------------------------------------------------------
std::vector<ClickHouseTagsConfig::Tag> ClickHouseTagsConfig::getTags( uniset3::ObjectId id )
{
...
void ClickHouseTagsConfig::Range::updateText( long value )
{
    for( auto&& t : tags )
    {
        ...
        if( needReplaceKey )
        {
            t.key = uniset3::replace_all(t.orig_key, "%v", std::to_string(value));
            t.key = uniset3::replace_all(t.key, "%min", std::to_string(vmin));
            t.key = uniset3::replace_all(t.key, "%max", std::to_string(vmax));
        ...
        if( needReplaceVal )
        {
            t.value = uniset3::replace_all(t.orig_value, "%v", std::to_string(value));
            t.value = uniset3::replace_all(t.value, "%min", std::to_string(vmin));
            t.value = uniset3::replace_all(t.value, "%max", std::to_string(vmax));
