...
// -------------------------------------------------------------------------
#if __cplusplus >= 201703L
uniset3::IDList uniset3::split_by_id( std::string_view str, char sep )
{
    return uniset3::IDList( split_sv(str, sep) );
}
// -------------------------------------------------------------------------
std::vector<std::string_view> uniset3::split_sv( std::string_view str, char sep )
{
...
#endif
// -------------------------------------------------------------------------
uniset3::IDList uniset3::split_by_id( const std::string& str, char sep )
{
...
// --------------------------------------------------------------------------------------
#if __cplusplus >= 201703L
bool uniset3::is_digit_sv( std::string_view s ) noexcept
{
    if( s.empty() )
        return false;
...
// --------------------------------------------------------------------------------------
std::list<uniset3::ParamSInfo> uniset3::getSInfoList_sv( std::string_view str, std::shared_ptr<uniset3::Configuration> conf )
{
    std::list<uniset3::ParamSInfo> res;

    auto lst = uniset3::split_sv(str, ',');

    for( auto&& it : lst )
    {
        uniset3::ParamSInfo item;

        auto p = uniset3::split_sv(it, '=');
        std::string_view s = "";
        ...
        if( t.size() == 1 )
        {
            ...
            if( is_digit_sv(s_id) || !conf )
                item.si.set_id(uni_atoi_sv(s_id));
            else
                item.si.set_id(conf->getSensorID(std::string(s_id)));
        ...
        else if( t.size() == 2 )
        {
            ...
            if( is_digit_sv(s_node) || !conf )
                item.si.set_node(uni_atoi_sv(s_node));
            else
                item.si.set_node(conf->getNodeID(std::string(s_node)));
        }
        else
        {
...
#endif
// --------------------------------------------------------------------------------------
std::list<uniset3::ParamSInfo> uniset3::getSInfoList( const std::string& str, std::shared_ptr<Configuration> conf )
{
    std::list<uniset3::ParamSInfo> res;
...
//--------------------------------------------------------------------------------------------
#if __cplusplus >= 201703L
int uniset3::uni_atoi_sv( std::string_view s ) noexcept
{
    if( s.empty() )
        return 0;
...
#endif
//--------------------------------------------------------------------------------------------
int uniset3::uni_atoi( const std::string& s ) noexcept
{
#if __cplusplus < 201703L
