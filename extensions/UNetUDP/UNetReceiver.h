...
namespace uniset3
{
     ...
     * map, ключом в котором является идентификатор данных (см. UDPmessages::getDataID()).
     * Кэш в map добавляется когда приходит пакет с новым UDPmessages::getDataID() и в дальнейшим используется для этого пакета.
     * В текущей реализации размер map не контролируется (завязан на UDPmessages::getDataID()) и рассчитан на статичность пакетов,
    ...
    */
        ...
        public std::enable_shared_from_this<UNetReceiver>
    {
        ...
        private:
            ...
            // ключом является UDPmessages::getDataID()
