...
namespace uniset3
{
    ...
    // -----------------------------------------------------------------------------
    OPCUAExchange::OPCUAExchange(uniset3::ObjectId id, uniset3::ObjectId icID,
    ...
    void OPCUAExchange::updateFromSM()
    {
        ...
        for( auto&& io : iolist )
        {
            ...
            try
            {
                if( io->stype == uniset3::AO )
                {
                    IOBase* ib = static_cast<IOBase*>(io.get());
                    {
                        uniset3::uniset_rwmutex_wrlock lock(io->vmut);
    ...
    void OPCUAExchange::updateToChannel( Channel* ch )
    {
        ...
        for( auto&& io : iolist )
        {
            ...
            try
            {
                ...
                {
                    {
                        uniset3::uniset_rwmutex_rlock lock(io->vmut);
    ...
    bool OPCUAExchange::initIOItem( UniXML::iterator& it )
    {
        ...
        if( vtype.empty() )
        {
            ...
            if( inf->stype == uniset3::DO || inf->stype == uniset3::DI )
                vtype = "bool";
        }
        ...
        for( size_t chan = 0; chan < numChannels; chan++ )
        {
            ...
            if( inf->stype == uniset3::AI || inf->stype == uniset3::DI )
            {
...
# if 0
    ...
    void OPCUAExchange::initOutputs()
    {
        ...
        for( const auto& it : iolist )
        {
            ...
            try
            {
                if( it->stype == uniset3::DO || it->stype == uniset3::AO )
                {
                    for( size_t i = 0; i < numChannels; i++ )
                    {
    ...
    // -----------------------------------------------------------------------------
    grpc::Status OPCUAExchange::getInfo(::grpc::ServerContext* context, const ::uniset3::GetInfoParams* request, ::google::protobuf::StringValue* response)
    {
    ...
    // -----------------------------------------------------------------------------
    void OPCUAExchange::askSensors( uniset3::UIOCommand cmd )
    {
        if( !shm->isLocalwork() )
            return;
        ...
        for( const auto& it : iolist )
        {
            ...
            if( it->stype == uniset3::AO || it->stype == uniset3::DO )
            {
                try
                {
    ...
    // -----------------------------------------------------------------------------
    void OPCUAExchange::timerInfo( const uniset3::TimerMessage* tm )
    {
        if( tm->id() == tmUpdates )
        {
            updateFromSM();
    ...
    // -----------------------------------------------------------------------------
    void OPCUAExchange::sensorInfo( const uniset3::SensorMessage* sm )
    {
        ...
        for( auto&& it : iolist )
        {
            if( it->si.id() == sm->id() )
            {
                if( it->stype == uniset3::AO )
                {
                    ...
                    ib->value = sm->value();
                    {
                        uniset3::uniset_rwmutex_wrlock lock(it->vmut);
