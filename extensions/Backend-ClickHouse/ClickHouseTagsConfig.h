...
namespace uniset3
{
    ...
    class ClickHouseTagsConfig
    {
        ...
        public:
            ...
            void load( const std::shared_ptr<uniset3::Configuration> conf
                       , const std::string& filter_field = ""
                       , const std::string& filter_value = "" );


            // init tags map from section <cnodename>
            void loadTagsMap( uniset3::UniXML::iterator it );

            // init tags for sensor
            void initFromItem( const std::shared_ptr<uniset3::Configuration> conf
            ...
            std::vector<Tag> getTags( uniset3::ObjectId id );

            bool updateTags( uniset3::ObjectId id, long value );

            size_t getTagsCount() const;
            size_t getSensorsCount() const;

            using StateMap = std::unordered_map<uniset3::ObjectId, long>;

            std::vector<uniset3::ObjectId> getAskedSensors() const;

        protected:

            // текущее значение датчиков по которым формируются теги (map<id,value>)
            StateMap::iterator addState( uniset3::ObjectId sid, long defval );
            ...
            RangeList initRangeList( uniset3::UniXML::iterator it );

            // name --> range list
            using TagsMap  = std::unordered_map<std::string, RangeList>;
            TagsMap tagsmap; // список тегов
            TagsMap initTagsMap( uniset3::UniXML::iterator it );
            ...
            TagList::iterator addTagsInfo( TagList& lst, uniset3::ObjectId sid );
            std::vector<Tag> makeTags( TagList& lst );

            // map<sensorID,TagList>
            std::unordered_map<uniset3::ObjectId, TagList> tags;

            void initDynamicTags();
            void initFromTags( const std::shared_ptr<uniset3::Configuration>& conf, uniset3::UniXML::iterator it, const std::string& s_tags );
            void initFromValues( const std::shared_ptr<uniset3::Configuration>& conf, uniset3::UniXML::iterator it, const std::string& s_values );
            void initFromTagsList( const std::shared_ptr<uniset3::Configuration>& conf, uniset3::UniXML::iterator it, uniset3::UniXML::iterator itList );
    };
    //------------------------------------------------------------------------------------------
