...
// -----------------------------------------------------------------------------
static std::shared_ptr<UInterface> ui = nullptr;
static std::shared_ptr<SMInterface> smi = nullptr;
// -----------------------------------------------------------------------------
extern std::shared_ptr<SharedMemory> shm;
extern std::shared_ptr<OPCUAExchange> opc;
extern std::shared_ptr<OPCUATestServer> opcTestServer1;
extern std::shared_ptr<OPCUATestServer> opcTestServer2;
// -----------------------------------------------------------------------------
...
TEST_CASE("OPCUAExchange: read", "[opcua][exchange][read]")
{
    ...
    REQUIRE(smi->getValue(sidAttr1) == 10);
    REQUIRE(smi->getValue(sidAttr2) == 1);
    ...
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidFloat1) == 1001);

    // float: round up
    opcTestServer1->setF32(rdFloat, 5.056);
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidFloat1) == 506);

    // float: round down
    opcTestServer1->setF32(rdFloat, 5.053);
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidFloat1) == 505);
}
// -----------------------------------------------------------------------------
TEST_CASE("OPCUAExchange: write", "[opcua][exchange][write]")
{
    ...
    REQUIRE_NOTHROW(smi->setValue(sidAttr3, 1000));
    REQUIRE_NOTHROW(smi->setValue(sidAttr4, 1));
    msleep(step_pause_msec);
    REQUIRE(opcTestServer1->getI32(wrAttr3) == 1000);
    REQUIRE(opcTestServer1->getBool(wrAttr4) == true);

    REQUIRE_NOTHROW(smi->setValue(sidAttr3, 20));
    REQUIRE_NOTHROW(smi->setValue(sidAttr4, 0));
    msleep(step_pause_msec);
    ...
    REQUIRE_NOTHROW(smi->setValue(sidFloatOut1, 2000));
    msleep(step_pause_msec);
    REQUIRE( opcTestServer1->getF32(wrFloatOut) == 20.0f );

    REQUIRE_NOTHROW(smi->setValue(sidFloatOut1, 2226));
    msleep(step_pause_msec);
...
TEST_CASE("OPCUAExchange: ignore", "[opcua][exchange][ignore]")
{
    ...
    REQUIRE( smi->getValue(sidAttr5) == 0 );
    msleep(step_pause_msec);
    REQUIRE( smi->getValue(sidAttr5) == 0 );
...
TEST_CASE("OPCUAExchange: read numeric nodeId", "[opcua][exchange][readI]")
{
    ...
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidAttrI101) == 10);

    opcTestServer1->setI32(rdI101, 20);
    REQUIRE(opcTestServer1->getI32(rdI101) == 20 );
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidAttrI101) == 20);
}
// -----------------------------------------------------------------------------
TEST_CASE("OPCUAExchange: read types", "[opcua][exchange][types]")
{
    ...
    REQUIRE_NOTHROW(smi->setValue(1022, 102)); // int16
    REQUIRE_NOTHROW(smi->setValue(1023, 103)); // uint16
    REQUIRE_NOTHROW(smi->setValue(1024, 104)); // int64
    REQUIRE_NOTHROW(smi->setValue(1025, 105)); // uint64
    REQUIRE_NOTHROW(smi->setValue(1026, 106)); // byte
    msleep(step_pause_msec);
...
TEST_CASE("MBTCPMaster: read/write mask", "[opcua][exchange][mask]")
{
    ...
    REQUIRE_NOTHROW(smi->setValue(sidAttr7, 1));
    msleep(step_pause_msec);
    REQUIRE(opcTestServer1->getI32(wrAttr7) == 1 );

    REQUIRE_NOTHROW(smi->setValue(sidAttr7, 3));
    msleep(step_pause_msec);
    REQUIRE(opcTestServer1->getI32(wrAttr7) == 3 );

    REQUIRE_NOTHROW(smi->setValue(sidAttr7, 5));
    msleep(step_pause_msec);
    REQUIRE(opcTestServer1->getI32(wrAttr7) == 1 );

    REQUIRE_NOTHROW(smi->setValue(sidAttr7, 8));
    msleep(step_pause_msec);
    REQUIRE(opcTestServer1->getI32(wrAttr7) == 0 );

    REQUIRE_NOTHROW(smi->setValue(sidAttr7, 2));
    msleep(step_pause_msec);
...
TEST_CASE("OPCUAExchange: exchangeMode", "[opcua][exchange][exchangemode]")
{
    ...
    SECTION("None")
    {
        ...
        SECTION("write")
        {
            REQUIRE_NOTHROW(smi->setValue(sidAttr3, 10));
            msleep(step_pause_msec);
    ...
    SECTION("WriteOnly")
    {
        ...
        SECTION("write")
        {
            REQUIRE_NOTHROW(smi->setValue(sidAttr3, 150));
            msleep(step_pause_msec);
            REQUIRE(opcTestServer1->getI32(wrAttr3) == 150);

            REQUIRE_NOTHROW(smi->setValue(sidAttr3, 155));
            msleep(step_pause_msec);
    ...
    SECTION("ReadOnly")
    {
        ...
        SECTION("write")
        {
            ...
            REQUIRE_NOTHROW(smi->setValue(sidAttr3, 250));
            msleep(step_pause_msec);
    ...
    SECTION("SkipSaveToSM")
    {
        ...
        SECTION("write")
        {
            ...
            REQUIRE_NOTHROW(smi->setValue(sidAttr3, 350));
            msleep(step_pause_msec);
...
TEST_CASE("OPCUAExchange: change channel", "[opcua][exchange][connection]")
{
    ...
    REQUIRE(smi->getValue(sidRespond) == 1);
    REQUIRE(smi->getValue(sidRespond1) == 1);
    REQUIRE(smi->getValue(sidRespond2) == 1);
    ...
    opcTestServer1->stop();
    msleep(timeout_msec);
    REQUIRE(smi->getValue(sidAttrI101) == 50);
    REQUIRE(smi->getValue(sidRespond) == 1);
    REQUIRE(smi->getValue(sidRespond1) == 0);
    REQUIRE(smi->getValue(sidRespond2) == 1);

    opcTestServer2->setI32(rdI101, 150);
    REQUIRE(opcTestServer2->getI32(rdI101) == 150 );
    msleep(step_pause_msec);
    REQUIRE(smi->getValue(sidAttrI101) == 150);
    ...
    opcTestServer2->stop();
    msleep(timeout_msec);
    REQUIRE(smi->getValue(sidRespond) == 0);
    REQUIRE(smi->getValue(sidRespond1) == 0);
    REQUIRE(smi->getValue(sidRespond2) == 0);
}
